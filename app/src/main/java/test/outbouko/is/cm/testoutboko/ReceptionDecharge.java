...
public class ReceptionDecharge extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.decharge_reception);
        thisActivity = this;
        ...
        if (isInternetOn()){
            if (Util.agences != null){
                ...
                spinner = (Spinner) findViewById(R.id.agences);
                ArrayAdapter<String> dataAdapter = new ArrayAdapter<String>(ReceptionDecharge.thisActivity,
                ...
                GetAgences getAgences = new GetAgences(ReceptionDecharge.thisActivity);
    ...
    public void getPartiSignature(View view){
        ...
        if (id == R.id.btn_signature_remetteur){
            requestCode = Util.REQUEST_CODE_REMETTEUR_SIGNATURE;
        }else if (id == R.id.btn_signature_recepteur){
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Util.REQUEST_CODE_REMETTEUR_SIGNATURE && resultCode == Activity.RESULT_OK){
            ...
            ImageView client_signature = (ImageView) findViewById(R.id.signature_remetteur);
        ...
        if (requestCode == Util.REQUEST_CODE_RECEPTEUR_SIGNATURE && resultCode == Activity.RESULT_OK){
            ...
            //Toast.makeText(getApplicationContext(),signature, Toast.LENGTH_LONG).show();
            byte[] b  = Base64.decode(signature, Base64.DEFAULT);
            ImageView agent_signature = (ImageView) findViewById(R.id.signature_recepteur);
            Bitmap bmp = BitmapFactory.decodeByteArray(b, 0, b.length);
    ...
    public void receiptDeliveries_(View view){
        String url = Util.BASE_URL +  "delivery/receipInCollectBase";
        String query = "email=" + email +
                        "&trackingcodes="+trackingCodes+
                        "&agence_id=" + selectedAgence.getId()+
                        "&collect_base_signature_user=" + remetteur_signature+
                        "&collect_base_signature_agent="+recepteur_signature;

        //Toast.makeText(ReceptionDecharge.thisActivity, query, Toast.LENGTH_LONG).show();
        Log.e("query receipt", query);
        ReceiptDelivery receiptDelivery = new ReceiptDelivery(ReceptionDecharge.thisActivity);
    ...
    private class ReceiptDelivery extends AsyncTask<String,Integer, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            AlertDialog alertDialog = new AlertDialog.Builder(ReceptionDecharge.this).create();
